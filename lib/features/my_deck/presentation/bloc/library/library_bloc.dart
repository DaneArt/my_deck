    ...
    yield* event.map(
        ...
        tryToStartTrain: (e) async* {
          final decksForTrain = await getDecksForTrain(
              Params(List<DeckLibrary>.from(state.decksSourceList)));
        ...
        updateDeck: (e) async* {
          final updateIndex = state.decksSourceList
              .indexWhere((d) => d.deckId == e.deck.deckId);
        ...
        undoEditing: (e) async* {
          final updateIndex = state.decksSourceList
              .indexWhere((d) => d.deckId == e.oldDeck.deckId);
          final decks = List<Deck>.from(state.decksSourceList);
          final updateResult = await saveDeckChangesUsecase(save.Params(decks[updateIndex],e.oldDeck));
